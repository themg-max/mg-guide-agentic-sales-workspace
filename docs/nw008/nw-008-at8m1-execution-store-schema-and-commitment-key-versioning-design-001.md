...
LATER_IMPLEMENTATION_WRITABLE_SCOPE_MAY_INCLUDE=
  ...
  tests/integrations/ghl/test_at1_execution_store.py
```
...
- If `schema_version` is older than the code's current version: apply only
  forward-only, store-owned migrations; no ad-hoc DDL outside the store.
...
- A narrow commitment-key provider interface is required, purpose-limited to
  execution-store commitment-key delivery only.
...
5. tests under `tests/integrations/ghl/test_at1_execution_store.py`
