...
SOURCE_DISCOVERABLE_IN_AUTHORIZED_SURFACES=VERIFIED|NOT_SURFACED|UNKNOWN
SOURCE_INSPECTABILITY=VERIFIED|UNKNOWN
...
It does not mean that the target does not exist. `SOURCE_INSPECTABILITY=UNKNOWN`
is used when a target was not surfaced. `NOT_AUTHORIZED_TO_INSPECT` means the
current unit has no grant to cross the public/private boundary for that source.
It is not a platform-capability conclusion.
...
SOURCE_DISCOVERABLE_IN_AUTHORIZED_SURFACES=NOT_SURFACED
SOURCE_INSPECTABILITY=UNKNOWN
AUTHORIZED_PUBLIC_AND_APPROVED_SOURCE_INSPECTION=YES
PRIVATE_REMAINDER_INSPECTION_AUTHORIZATION=NOT_AUTHORIZED_TO_INSPECT
SEARCH_EVIDENCE_SOURCE_CLASSES=REPO_CANONICAL|MG_MCP_REPO_REVIEW|MG_MCP_APPROVED_DOC
TARGET_SOURCE_CLASS=UNKNOWN
...
SOURCE_DISCOVERABLE_IN_AUTHORIZED_SURFACES=VERIFIED
SOURCE_INSPECTABILITY=VERIFIED
AUTHORIZED_PUBLIC_AND_APPROVED_SOURCE_INSPECTION=YES
PRIVATE_REMAINDER_INSPECTION_AUTHORIZATION=NOT_AUTHORIZED_TO_INSPECT
SEARCH_EVIDENCE_SOURCE_CLASSES=REPO_CANONICAL
TARGET_SOURCE_CLASS=REPO_CANONICAL
...
NEXT_AUTHORIZATION_MUST_BE_METADATA_ONLY=YES
EXACT_HUMAN_PRINCIPAL_VALUE_OUT_OF_SCOPE=YES
PRIVATE_AUTHORITY_RECORD_CONTENT_OUT_OF_SCOPE=YES
...
EXISTING_INGESTION_CONTROLLER_PRIVATE_PII_AUTHORITY=NO
POSITIVE_REUSE_CLAIM_FOR_PRIVATE_PII=CONTRADICTED
```
...
AUTHORIZED_PUBLIC_AND_APPROVED_SOURCE_INSPECTION=YES
PRIVATE_REMAINDER_INSPECTION_AUTHORIZATION=NOT_AUTHORIZED_TO_INSPECT
SEARCH_EVIDENCE_SOURCE_CLASSES=DECISION_HISTORY
TARGET_SOURCE_CLASS=REPO_CANONICAL
...
| --- | --- | --- | --- | --- | --- | --- |
| Canonical exact request source | `UNKNOWN` | `NOT_SURFACED` | `UNKNOWN` | `YES` | `UNKNOWN` | Source path/version/complete fields remain unknown |
| Existing exact behavior summary | `VERIFIED` | `VERIFIED` | `VERIFIED` | `YES` | `REPO_CANONICAL` | Partial behavior only; not the exact request schema |
| Existing private control-plane class | `VERIFIED` | `VERIFIED` | `UNKNOWN` | `YES` | `PRIVATE_SOURCE_METADATA` | Class exists; identifier and reuse fit unresolved |
| Selected private authority system | `UNKNOWN` | `NOT_SURFACED` | `UNKNOWN` | `NOT_AUTHORIZED_TO_INSPECT` | `PRIVATE_SOURCE_METADATA` | Candidate design is not a selected source |
| Private owner/access model | `UNKNOWN` | `NOT_SURFACED` | `UNKNOWN` | `NOT_AUTHORIZED_TO_INSPECT` | `PRIVATE_SOURCE_METADATA` | Concrete owner and access model unresolved |
| Public/private boundary policy | `VERIFIED` | `VERIFIED` | `VERIFIED` | `YES` | `REPO_CANONICAL` | Exact human principal remains private |
| Existing ingestion private-PII authority | `VERIFIED` | `VERIFIED` | `VERIFIED` | `YES` | `REPO_CANONICAL` | Current authority claim is `NO`; positive reuse claim is contradicted |
| Operational private-PII grant | `UNKNOWN` | `NOT_SURFACED` | `UNKNOWN` | `NOT_AUTHORIZED_TO_INSPECT` | `PRIVATE_SOURCE_METADATA` | No usable operational grant evidenced |
| In-process placement compute | `VERIFIED` | `VERIFIED` | `VERIFIED` | `YES` | `REPO_CANONICAL` | Existing process can host placement; implementation fit unknown |
| End-to-end private infrastructure | `UNKNOWN` | `NOT_SURFACED` | `UNKNOWN` | `NOT_AUTHORIZED_TO_INSPECT` | `PRIVATE_SOURCE_METADATA` | Infra class remains unknown |
| Runtime identity decision | `VERIFIED` | `VERIFIED` | `VERIFIED` | `YES` | `DECISION_HISTORY` | Existing identity constraints preserved |
| Private retrieval IAM | `UNKNOWN` | `NOT_SURFACED` | `UNKNOWN` | `NOT_AUTHORIZED_TO_INSPECT` | `PRIVATE_SOURCE_METADATA` | IAM class remains unknown |
...
PRIVATE_AUTHORITY_SOURCE_INSPECTION_STATUS=UNKNOWN
...
AT8O10_DEFINED_PUBLIC_AND_APPROVED_SEARCH_PLAN_EXHAUSTED=YES
FURTHER_SAME_CLASS_SEARCH_EXPECTED_INFORMATION_GAIN=LOW
