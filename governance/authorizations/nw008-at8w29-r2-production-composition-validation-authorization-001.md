...
Before target-runtime credential object construction or any Secret Manager
call, the execution consumer must perform the following checks only, in order,
and must not mutate filesystem state.
...
REQUIRE_BEFORE_TARGET_RUNTIME_CREDENTIAL_OBJECT_CONSTRUCTION=
...
AUTHORIZATION_STATE_ON_FIRST_TARGET_RUNTIME_CREDENTIAL_OBJECT_CONSTRUCTION_ATTEMPT=
  CONSUMED
AUTHORIZATION_STATE_AFTER_FIRST_TARGET_RUNTIME_CREDENTIAL_OBJECT_CONSTRUCTION_ATTEMPT=
  CONSUMED

TARGET_RUNTIME_CREDENTIAL_OBJECT_CONSTRUCTION != TOKEN_MINT
SERVICE_ACCOUNT_IMPERSONATION_ATTEMPTS=
  SEPARATELY_RECORDED_LIVE_COUNTER
SERVICE_ACCOUNT_ACCESS_TOKEN_MINTS=
  SEPARATELY_RECORDED_LIVE_COUNTER
...
target-runtime credential object construction attempt, regardless of success or
failure. Credential object construction is distinct from token minting.
Service-account impersonation attempts and access-token mints remain separately
recorded live counters. Once target credential construction is attempted,
...
closed before any target-runtime credential object construction attempt with
...
Before the first target-runtime credential object construction attempt, the
execution consumer must independently verify:
...
OPERATOR_COMPENSATING_CLEANUP_MUTATION_ON_FAILURE=NO
CONSUMER_COMPENSATING_CLEANUP_MUTATION_ON_FAILURE=NO
ROOT_OWNED_STORE_CONNECTION_CLOSE_ON_PRE_RETURN_FAILURE=REQUIRED
ROOT_OWNED_STORE_CONNECTION_CLOSE_IS_FORBIDDEN_COMPENSATING_MUTATION=NO
...
If failure occurs after the first target-runtime credential object construction
attempt:
...
AUTHORIZATION_CONSUMPTION_TRIGGER=
  FIRST_TARGET_RUNTIME_CREDENTIAL_OBJECT_CONSTRUCTION_ATTEMPT
TARGET_RUNTIME_CREDENTIAL_OBJECT_CONSTRUCTION != TOKEN_MINT
SERVICE_ACCOUNT_IMPERSONATION_ATTEMPTS_LIVE_COUNTER=YES
SERVICE_ACCOUNT_ACCESS_TOKEN_MINTS_LIVE_COUNTER=YES
