# NW-008 AT1 GHL REST v3 Bounded-Read Authorization 004
...
AUTHORIZATION_BRANCH=
  authorization/nw008-at1-ghl-rest-v3-bounded-read-authorization-004
...
Authorization/Activation 003 are not authority for this unit and must not be
reused, revived, or transferred. Authorization 003 bound its execution to the
`MG_GUIDE_PIT_GHL/versions/1` credential; that attempt terminated on a
provider 403 (`proof/nw008/nw-008-at1-ghl-rest-v3-provider-403-root-cause-diagnostic-001.md`).
The underlying credential was subsequently rotated
...
and the active runtime pin was moved from v1 to v2 in PR #370. This is a
materially different credential and consumption state and requires fresh
authorization and activation rather than reuse of 003.
...
identifiers remain excluded from this public repository. This binding is
unchanged since Authorization 003; only the credential version consumed by
section 3.2 has changed.
...
PRIOR_CREDENTIAL_VERSION_STATE=DISABLED
CURRENT_CREDENTIAL_VERSION=2
CURRENT_CREDENTIAL_VERSION_STATE=ENABLED
RUNTIME_PIN_SOURCE_PR=370
RUNTIME_PIN_SOURCE_SHA=532e0f7cb4114268e394241b264a432e0d91cc17
```
...
mismatch is terminal and permits no retry. A repeat 403 or any other terminal
outcome on this unit is a fresh diagnostic and rotation question, not grounds
for a second attempt under this authorization.
...
```text
PROOF_001_MERGED_TO_MAIN=YES
PROOF_001_MERGE_PRESENT_ON_ORIGIN_MAIN=YES
PIT_ROTATION_PROOF_MERGED_TO_MAIN=YES
RUNTIME_V2_PIN_MERGED_TO_MAIN=YES
RUNTIME_V2_PIN_PR=370
RUNTIME_V2_PIN_SHA_ANCESTOR_OF_MAIN=YES
PRIVATE_BINDING_BLOB_SHA_MATCH=REQUIRED
FRESH_HUMAN_ACTIVATION_004_EFFECTIVE=REQUIRED
