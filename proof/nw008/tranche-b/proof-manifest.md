...
| TB-10 | PASS | `proof/nw008/tranche-b/context-delta.json` | Every confirmed-context claim class (current facts, unresolved questions, commitments completed/open, proposed next step) retains evidence references. | none |
...
| TB-12 | PASS | `proof/nw008/tranche-b/meeting-2-run.json` | Deterministic policy invoked, received relationship_context.longitudinal_context, and bypass was false. | none |
| TB-13 | PASS | `proof/nw008/tranche-b/decision-card.json` | NW-007 decision card rendered non-empty text/html, zero external effects, valid next_action, and known policy state. | none |
...
| TB-17 | PASS | `proof/nw008/tranche-b/meeting-1-run.json + proof/nw008/tranche-b/meeting-2-run.json` | Synthetic fixture safety validation passed: source=synthetic_demo, approved test domain, NW008 namespace, zero live CRM calls/writes/real_customer_data. | none |
