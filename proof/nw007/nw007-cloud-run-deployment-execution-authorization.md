...
| ARTIFACT_REGISTRY_CREATION_REQUIRED | YES (conditional) — no repository could be verified; creation would be required before any image push, under explicit authorization. | Above; no registry config anywhere in repo. |
| BUILD_SERVICE_ACCOUNT | UNKNOWN — Cloud Build has never been enabled in this project, so the Cloud Build service agent (`985566250549@cloudbuild.gserviceaccount.com`) is not provisioned. Proposed: dedicated least-privilege build SA (e.g. `mg-guide-devpost-build@mg-devpost.iam.gserviceaccount.com`), NOT created. | `gcloud builds list --project=mg-devpost` → PERMISSION_DENIED "Cloud Build has not been used in project 985566250549 before or it is disabled." |
...
| DEPLOYER_PRINCIPAL | PROPOSED: `user:themg@themiliare-group.com` (currently the sole `roles/owner` on `mg-devpost` and the only principal able to deploy today). Final deployer identity must be confirmed by the human approver; this artifact does not grant or assume deployment rights. | `gcloud projects get-iam-policy mg-devpost` (read-only) — single owner binding plus Google-managed firebaserules/firestore service agents. |
...
STOP_CODE=NW007_EXECUTION_AUTHORIZATION_REVISED_READY_FOR_HUMAN_REVIEW
